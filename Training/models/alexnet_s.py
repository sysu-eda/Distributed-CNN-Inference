...
class AlexNet_S(nn.Module):
    def __init__(self, num_classes=1000, num_group=4):
        ...
        self.GConv2 = nn.Sequential(
            nn.Conv2d(64, 192, kernel_size=5, padding=2, groups=num_group), # shape is 27 x 27 x 192
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2), # shape is 13 x 13 x 192
        )
        
        self.GConv3 = nn.Sequential(
            nn.Conv2d(192, 384, kernel_size=3, padding=1, groups=num_group), # shape is 13 x 13 x 384
            nn.ReLU(inplace=True)
        )
        ...
        self.GConv5 = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, padding=1, groups=num_group), # shape is 13 x 13 x 256
            nn.ReLU(inplace=True),
