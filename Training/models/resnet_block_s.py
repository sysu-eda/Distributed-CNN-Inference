...
class Bottleneck(nn.Module):
    ...
    def channel_shuffle(self, x):
        ...
        x = x.reshape(batchsize, group_channels, 4, height, width)
        x = x.permute(0, 2, 1, 3, 4)
        x = x.reshape(batchsize, num_channels, height, width)
        return x


class ResNet_Block_S(nn.Module):

    def __init__(self, block, layers, num_classes=1000):
        super(ResNet_Block_S, self).__init__()
...
def resnet50_block_s(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_Block_S(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
