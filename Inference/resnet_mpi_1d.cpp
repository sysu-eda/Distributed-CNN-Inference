...
void partlyGather(void *sendbuf, void *recvbuf, MPI_Comm comm, int rank, int size, int count, MPI_Request array_of_requests[])
{
	...
	//cout<<rank<<"1"<<endl;
	if(rank>0){
		MPI_Irecv((uint8_t*)recvbuf, count, MPI_FLOAT, rank-1, 100, comm, array_of_requests);//+count*i*16
		array_of_requests++;
	}
	//cout<<rank<<"2"<<endl;
	if(rank<3){
		MPI_Irecv((uint8_t*)recvbuf+count*4, count, MPI_FLOAT, rank+1, 100, comm, array_of_requests);//+count*i*16
		array_of_requests++;
	}
	//cout<<rank<<"3"<<endl;
	if(rank>0)
	{
		MPI_Isend((uint8_t*)sendbuf, count, MPI_FLOAT, rank-1, 100, comm, &request_tmp);//+count*j*16
	}
	//cout<<rank<<"4"<<endl;
	if(rank<3)
	{
		MPI_Isend((uint8_t*)sendbuf+count*4, count, MPI_FLOAT, rank+1, 100, comm, &request_tmp);//+count*j*16
	}
...
int main (int argc, char **argv)
{
	...
	Tensor * pool1_out = opWrapper::configure3DTensor(56,16,64);
	//Layer0
	Tensor * layer0_block0_conv0_out = new Tensor();
	Tensor * layer0_block0_bn0_out = opWrapper::configure3DTensor(56,16,64);
	...
	Tensor * layer0_block1_bn0_out = opWrapper::configure3DTensor(56,16,64);
	...
	Tensor * layer0_block2_bn0_out = opWrapper::configure3DTensor(56,16,64);
	...
	Tensor * layer1_block0_bn0_out = opWrapper::configure3DTensor(56,16,128);
	...
	Tensor * layer1_block1_bn0_out = opWrapper::configure3DTensor(28,8,128);
	...
	Tensor * layer1_block2_bn0_out = opWrapper::configure3DTensor(28,8,128);
	...
	Tensor * layer1_block3_bn0_out = opWrapper::configure3DTensor(28,8,128);
	...
	Tensor * layer2_block0_bn0_out = opWrapper::configure3DTensor(28,8,256);
	...
	Tensor * layer2_block1_bn0_out = opWrapper::configure3DTensor(14,5,256);
	...
	Tensor * layer2_block2_bn0_out = opWrapper::configure3DTensor(14,5,256);
	...
	Tensor * layer2_block3_bn0_out = opWrapper::configure3DTensor(14,5,256);
	...
	Tensor * layer2_block4_bn0_out = opWrapper::configure3DTensor(14,5,256);
	...
	Tensor * layer2_block5_bn0_out = opWrapper::configure3DTensor(14,5,256);
	...
	Tensor * layer3_block0_bn0_out = opWrapper::configure3DTensor(14,5,512);
	...
	Tensor * layer3_block1_bn0_out = opWrapper::configure3DTensor(7,4,512);
	...
	Tensor * layer3_block2_bn0_out = opWrapper::configure3DTensor(7,4,512);
	...
	//Define Input Tensor
	pmLoader ppm;
	ppm.open("/home/pi/NeurIoT_mpi/go_kart_quarter.ppm");	//
	...
	void *SendTmp = (void*)(new float[8000]);
	void *pTmp = (void*)(new float[8000]);
	
	int commArr[17] = {2,5,14,21,28,37,44,51,58,67,74,81,88,95,102,111,118};
	int sizeofTrans = 4000;	
	int commsize = 2;
	...
	MPI_Request arrReq0[commsize];
	MPI_Status array_of_statuses[commsize];
	...
	while(iters<atoi(argv[2]))
	{
		...
		for(int i = 0; i<layer_num; i++){
			...
			if(i == commArr[i_comm])
			{
				...
				{
					usleep(5);
					MPI_Testall(commsize, arrReq0, &flag0, array_of_statuses);
				}
				flag0 = 0;
				i_comm++;
			}
