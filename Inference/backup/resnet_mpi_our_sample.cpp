...
int main (int argc, char **argv)
{
	...
	if(argc != 5)
	{
		std::cout<<"Usage: mpiexec -hostfile [hosts] -np [4] -host [raspberrypi0,raspberrypi1] ./main [numberThread(1)] [numberIteration(100) [scale] [intersample]]"<<std::endl;
		return 0;
	}	
	...
	while(iters<atoi(argv[2]))
	{
		...
		/* if(rank == 0)
		{
			std::cout<<"Queue0:"<<std::endl;
			for(int m = 0; m<125; m++)
			{
				std::cout<<queue0[m]<<"\t";
			}
			std::cout<<std::endl<<"Queue1:"<<std::endl;
			for(int m = 0; m<125; m++)
			{
				std::cout<<queue1[m]<<"\t";	
			}
